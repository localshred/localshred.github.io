...
Elm.UI.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var pureButton = F2(function (onClickHandler,buttonText) {
      return A2($Html.button,_U.list([$Html$Attributes.$class("pure-button"),onClickHandler]),_U.list([$Html.text(buttonText)]));
   });
   return _elm.UI.values = {_op: _op,pureButton: pureButton};
};
Elm.Games = Elm.Games || {};
Elm.Games.Minesweeper = Elm.Games.Minesweeper || {};
Elm.Games.Minesweeper.make = function (_elm) {
   "use strict";
   _elm.Games = _elm.Games || {};
   _elm.Games.Minesweeper = _elm.Games.Minesweeper || {};
   if (_elm.Games.Minesweeper.values) return _elm.Games.Minesweeper.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   $UI = Elm.UI.make(_elm);
   var _op = {};
   var neighbors = F2(function (_p1,_p0) {
      var _p2 = _p1;
      var _p3 = _p0;
      var _p9 = _p3._0;
      var _p8 = _p3._1;
      var neighborFilter = function (_p4) {
         var _p5 = _p4;
         var _p7 = _p5._0;
         var _p6 = _p5._1;
         return _U.cmp(_p7,-1) > 0 && (_U.cmp(_p6,-1) > 0 && (_U.cmp(_p7,_p2._1) < 0 && _U.cmp(_p6,_p2._0) < 0));
      };
      var right = _p8 + 1;
      var center = _p8;
      var left = _p8 - 1;
      ...
      var middle = _p9;
      var top = _p9 - 1;
      var candidates = _U.list([{ctor: "_Tuple2",_0: top,_1: left}
                               ,{ctor: "_Tuple2",_0: top,_1: center}
                               ,{ctor: "_Tuple2",_0: top,_1: right}
                               ,{ctor: "_Tuple2",_0: middle,_1: left}
                               ,{ctor: "_Tuple2",_0: middle,_1: center}
                               ,{ctor: "_Tuple2",_0: middle,_1: right}
                               ,{ctor: "_Tuple2",_0: bottom,_1: left}
                               ,{ctor: "_Tuple2",_0: bottom,_1: center}
                               ,{ctor: "_Tuple2",_0: bottom,_1: right}]);
      return A2($List.filter,neighborFilter,candidates);
   });
   var cellNumberToSquarePosition = F2(function (width,cell) {
      var col = A2($Basics._op["%"],cell,width);
      var row = cell / width | 0;
      return A2(F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),row,col);
   });
   var mineLocationGenerator = F2(function (count,maxSize) {    return A2($Random.list,count,A2($Random.$int,0,maxSize + 1));});
   var dec = function (v) {    return v - 1;};
   var generateRandomMineCells = F4(function (count,maxSize,seed,initialMines) {
      generateRandomMineCells: while (true) {
         var randomMines = A2($Set.union,
         initialMines,
         $Set.fromList(A2($List.map,dec,$List.sort($Basics.fst(A2($Random.generate,A2(mineLocationGenerator,count,maxSize),$Random.initialSeed(seed)))))));
         var missingMinesCount = count - $Set.size(randomMines);
         if (_U.cmp(missingMinesCount,0) > 0) {
               var _v3 = missingMinesCount,_v4 = maxSize,_v5 = seed + 1,_v6 = randomMines;
               count = _v3;
               maxSize = _v4;
               seed = _v5;
               initialMines = _v6;
               continue generateRandomMineCells;
            } else return randomMines;
      }
   });
   var selectMode = F2(function (mode,model) {
      var _p10 = mode;
      switch (_p10.ctor)
      {case "Beginner": return _U.update(model,{mode: mode,dimensions: {ctor: "_Tuple2",_0: 9,_1: 9},mineCount: 10});
         case "Intermediate": return _U.update(model,{mode: mode,dimensions: {ctor: "_Tuple2",_0: 16,_1: 16},mineCount: 40});
         default: return _U.update(model,{mode: mode,dimensions: {ctor: "_Tuple2",_0: 30,_1: 16},mineCount: 99});}
   });
   var eqSquare = F2(function (a,b) {    return _U.eq(a,b);});
   var updateElapsedTime = function (model) {
      var _p11 = model.state;
      if (_p11.ctor === "Started") {
            return _U.cmp(model.startTime,0) > 0 && _U.cmp(model.currentTime,0) > 0 ? _U.update(model,
            {elapsedTime: $Basics.floor((model.currentTime - model.startTime) / 1000)}) : model;
         } else {
            return model;
         }
   };
   var initializeStartTime = function (model) {
      var _p12 = model.state;
      if (_p12.ctor === "Started") {
            return _U.eq(model.startTime,0) && _U.cmp(model.currentTime,0) > 0 ? _U.update(model,{startTime: model.currentTime}) : model;
         } else {
            return model;
         }
   };
   var marker = function (square) {
      var _p13 = square.visibility;
      switch (_p13.ctor)
      {case "Flagged": if (_p13._0.ctor === "Flag") {
                 return "🚩";
              } else {
                 return "❓";
              }
         case "Incorrect": return "❌";
         case "Uncovered": var _p14 = square.content;
           switch (_p14.ctor)
           {case "Touching": var _p15 = _p14._0;
                return _U.eq(_p15,0) ? "" : $Basics.toString(_p15);
              case "Mine": return "💣";
              default: return "💥";}
         case "Covered": return "";
         default: return "";}
   };
   var isTouchingMoreThanZero = function (square) {
      var _p16 = square.content;
      if (_p16.ctor === "Touching") {
            if (_p16._0 === 0) {
                  return false;
               } else {
                  return true;
   ...
   var padNumber = function (n) {
      return _U.cmp(n,10) < 0 ? A2($Basics._op["++"],"00",$Basics.toString(n)) : _U.cmp(n,100) < 0 ? A2($Basics._op["++"],
      "0",
      $Basics.toString(n)) : $Basics.toString(n);
   };
   var Model = F9(function (a,b,c,d,e,f,g,h,i) {
      return {state: a,mode: b,dimensions: c,mineCount: d,board: e,metaKeyDown: f,startTime: g,currentTime: h,elapsedTime: i};
   });
   var UncoverNeighborSquares = function (a) {    return {ctor: "UncoverNeighborSquares",_0: a};};
   var PeekSquare = function (a) {    return {ctor: "PeekSquare",_0: a};};
   var SelectSquare = function (a) {    return {ctor: "SelectSquare",_0: a};};
   var ModeSelect = function (a) {    return {ctor: "ModeSelect",_0: a};};
   var Tick = function (a) {    return {ctor: "Tick",_0: a};};
   var MetaKeyDown = function (a) {    return {ctor: "MetaKeyDown",_0: a};};
   var inputs = $Signal.mergeMany(_U.list([A2($Signal.map,MetaKeyDown,$Keyboard.meta),A2($Signal.map,Tick,$Time.every($Time.second))]));
   var Restart = {ctor: "Restart"};
   var restartButton = F2(function (address,model) {
      var restartEmoji = function () {
      ...
      return A2($Html.div,_U.list([$Html$Attributes.$class("row")]),_U.list([A2($UI.pureButton,A2($Html$Events.onClick,address,Restart),restartEmoji)]));
   });
   var Advanced = {ctor: "Advanced"};
   var Intermediate = {ctor: "Intermediate"};
   var Beginner = {ctor: "Beginner"};
   var pendingView = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "text-align",_1: "center"}]))]),
      _U.list([$Html.text("Start Game: ")
              ,A2($UI.pureButton,A2($Html$Events.onClick,address,ModeSelect(Beginner)),"Beginner")
              ,A2($UI.pureButton,A2($Html$Events.onClick,address,ModeSelect(Intermediate)),"Intermediate")
              ,A2($UI.pureButton,A2($Html$Events.onClick,address,ModeSelect(Advanced)),"Advanced")]));
   });
   var Loss = {ctor: "Loss"};
   var Win = {ctor: "Win"};
   var Started = {ctor: "Started"};
   var Pending = {ctor: "Pending"};
   var init = A9(Model,Pending,Beginner,{ctor: "_Tuple2",_0: 0,_1: 0},0,_U.list([]),false,0,0,0);
   var Square = F3(function (a,b,c) {    return {pos: a,content: b,visibility: c};});
   var Incorrect = {ctor: "Incorrect"};
   var Peek = {ctor: "Peek"};
   var Flagged = function (a) {    return {ctor: "Flagged",_0: a};};
   var Uncovered = {ctor: "Uncovered"};
   var shouldAddPeekHandlers = F2(function (model,square) {
      return _U.eq(model.state,Started) && (isTouchingMoreThanZero(square) && _U.eq(square.visibility,Uncovered));
   });
   var uncoverSquare = F2(function (selectedSquare,square) {
      return _U.eq(selectedSquare.pos,square.pos) ? _U.update(square,{visibility: Uncovered}) : square;
   });
   var uncoverSquareInBoard = F2(function (model,square) {
      var updateRow = function (row) {    return A2($List.map,uncoverSquare(square),row);};
      var updatedBoard = A2($List.map,updateRow,model.board);
      return _U.update(model,{board: updatedBoard});
   });
   var uncoverMatchingSquares = F2(function (squares,square) {
      return A2($List.member,square,squares) ? _U.update(square,{visibility: Uncovered}) : square;
   });
   var Covered = {ctor: "Covered"};
   var uncoverSquareNeighbors = F2(function (model,square) {
      var linkedNeighborPositions = A2(neighbors,model.dimensions,square.pos);
      var findNeighborSquares = function (square$) {    return A2($List.member,square$.pos,linkedNeighborPositions) && _U.eq(square$.visibility,Covered);};
      var neighborSquares = A2($List.filter,findNeighborSquares,$List.concat(model.board));
      var updateRow = function (row) {    return A2($List.map,uncoverMatchingSquares(neighborSquares),row);};
      var updatedBoard = A2($List.map,updateRow,model.board);
      return _U.update(model,{board: updatedBoard});
   });
   var peekMatchingSquares = F2(function (squares,square) {
      return A2($List.member,square,squares) && _U.eq(square.visibility,Covered) ? _U.update(square,{visibility: Peek}) : A2($List.member,
      square,
      squares) && _U.eq(square.visibility,Peek) ? _U.update(square,{visibility: Covered}) : square;
   });
   var peekSquareNeighbors = F2(function (model,square) {
      var linkedNeighborPositions = A2(neighbors,model.dimensions,square.pos);
      var findNeighborSquares = function (square$) {    return A2($List.member,square$.pos,linkedNeighborPositions);};
      var neighborSquares = A2($List.filter,findNeighborSquares,$List.concat(model.board));
      var updateRow = function (row) {    return A2($List.map,peekMatchingSquares(neighborSquares),row);};
      var updatedBoard = A2($List.map,updateRow,model.board);
      return _U.update(model,{board: updatedBoard});
   });
   var Question = {ctor: "Question"};
   var Flag = {ctor: "Flag"};
   var score = function (model) {
      var flaggedCount = function () {
         var _p18 = model.state;
         switch (_p18.ctor)
         {case "Started": return A2(F2(function (x,y) {    return x - y;}),
              model.mineCount,
              $List.length(A2($List.filter,function (square) {    return _U.eq(square.visibility,Flagged(Flag));},$List.concat(model.board))));
            case "Win": return 0;
            default: return model.mineCount;}
      }();
      var stringFlagCount = _U.cmp(flaggedCount,10) < 0 ? A2($Basics._op["++"],"00",$Basics.toString(flaggedCount)) : _U.cmp(flaggedCount,
      100) < 0 ? A2($Basics._op["++"],"0",$Basics.toString(flaggedCount)) : $Basics.toString(flaggedCount);
      return A2($Html.div,_U.list([$Html$Attributes.$class("score")]),_U.list([$Html.text(stringFlagCount)]));
   };
   var squareNeighborFlagsEqualTouchingCount = F2(function (model,square) {
      if (_U.eq(square.visibility,Uncovered) && isTouchingMoreThanZero(square)) {
            var _p19 = square.content;
            if (_p19.ctor === "Touching") {
                  var linkedNeighborPositions = A2(neighbors,model.dimensions,square.pos);
                  var findNeighborSquares = function (square$) {
                     return A2($List.member,square$.pos,linkedNeighborPositions) && _U.eq(square$.visibility,Flagged(Flag));
                  };
                  var flaggedCount = $List.length(A2($List.filter,findNeighborSquares,$List.concat(model.board)));
                  return _U.eq(_p19._0,flaggedCount);
               } else {
                  return false;
               }
         } else return false;
   });
   var flaggedNeighboringMineCount = F2(function (model,square) {
      return $List.length(A2($List.filter,function (square) {    return _U.eq(square.visibility,Flagged(Flag));},$List.concat(model.board)));
   });
   var flagSquare = F2(function (selectedSquare,square) {
      if (!_U.eq(selectedSquare.pos,square.pos)) return square; else {
            var _p20 = selectedSquare.visibility;
            switch (_p20.ctor)
            {case "Covered": return _U.update(square,{visibility: Flagged(Flag)});
               case "Flagged": if (_p20._0.ctor === "Flag") {
                       return _U.update(square,{visibility: Flagged(Question)});
                    } else {
                       return _U.update(square,{visibility: Covered});
                    }
               default: return square;}
         }
   });
   var flagSquareInBoard = F2(function (model,square) {
      var updateRow = function (row) {    return A2($List.map,flagSquare(square),row);};
      var updatedBoard = A2($List.map,updateRow,model.board);
      return _U.update(model,{board: updatedBoard});
   });
   var ExplodedMine = {ctor: "ExplodedMine"};
   var mineExploded = F2(function (model,square) {
      var squareExploded = function (square$) {
         return A2(eqSquare,square.pos,square$.pos) ? _U.update(square,{content: ExplodedMine,visibility: Uncovered}) : _U.update(square$,
         {visibility: Uncovered});
      };
      var updateRow = function (row) {    return A2($List.map,squareExploded,row);};
      var updatedBoard = A2($List.map,updateRow,model.board);
      return _U.update(model,{board: updatedBoard,state: Loss});
   });
   var Mine = {ctor: "Mine"};
   var flagAllMines = function (rows) {
      var flagMineSquare = function (square) {    return _U.eq(square.content,Mine) ? _U.update(square,{visibility: Flagged(Flag)}) : square;};
      var updateRow = function (squares) {    return A2($List.map,flagMineSquare,squares);};
      var rows$ = A2($List.map,updateRow,rows);
      return rows$;
   };
   var promoteToWin = function (model) {
      var squareCount = $Basics.fst(model.dimensions) * $Basics.snd(model.dimensions);
      var uncoveredSquareCountTriggeringWin = squareCount - model.mineCount;
      var uncoveredCount = $List.length(A2($List.filter,function (square) {    return _U.eq(square.visibility,Uncovered);},$List.concat(model.board)));
      var model$ = _U.eq(uncoveredCount,uncoveredSquareCountTriggeringWin) ? _U.update(model,{state: Win,board: flagAllMines(model.board)}) : model;
      return model$;
   };
   var uncoverAllSquaresAndShowIncorrectFlags = function (rows) {
      var flagMineSquare = function (square) {
         return !_U.eq(square.content,Mine) && _U.eq(square.visibility,Flagged(Flag)) ? _U.update(square,{visibility: Incorrect}) : !_U.eq(square.content,
         Mine) && _U.eq(square.visibility,Flagged(Question)) ? _U.update(square,{visibility: Incorrect}) : _U.eq(square.visibility,
         Covered) || _U.eq(square.visibility,Peek) ? _U.update(square,{visibility: Uncovered}) : square;
      };
      var updateRow = function (squares) {    return A2($List.map,flagMineSquare,squares);};
      var rows$ = A2($List.map,updateRow,rows);
      return rows$;
   };
   var promoteToLoss = function (model) {
      var uncoveredMinesCount = $List.length(A2($List.filter,
      function (square) {
         return _U.eq(square.visibility,Uncovered) && _U.eq(square.content,Mine);
      },
      $List.concat(model.board)));
      var model$ = _U.cmp(uncoveredMinesCount,0) > 0 ? _U.update(model,
      {state: Loss,elapsedTime: 999,board: uncoverAllSquaresAndShowIncorrectFlags(model.board)}) : model;
      return model$;
   };
   var promoteToWinOrLoss = function (model) {
      var winModel = promoteToWin(model);
      var lossModel = promoteToLoss(model);
      return _U.eq(lossModel.state,Loss) ? lossModel : _U.eq(winModel.state,Win) ? winModel : model;
   };
   var Touching = function (a) {    return {ctor: "Touching",_0: a};};
   var printSquare = F3(function (address,model,square) {
      var uncoverNeighborsHandler = A2($Html$Events.onClick,address,UncoverNeighborSquares(square));
      var peekUpHandler = A2($Html$Events.onMouseUp,address,PeekSquare(square));
      var peekDownHandler = A2($Html$Events.onMouseDown,address,PeekSquare(square));
      var uncoverSquareHandler = A2($Html$Events.onClick,address,SelectSquare(square));
      var classes = $Html$Attributes.classList(_U.list([{ctor: "_Tuple2",_0: "square",_1: true}
                                                       ,{ctor: "_Tuple2",_0: "flagged flagged-flag",_1: _U.eq(square.visibility,Flagged(Flag))}
                                                       ,{ctor: "_Tuple2",_0: "flagged flagged-question",_1: _U.eq(square.visibility,Flagged(Question))}
                                                       ,{ctor: "_Tuple2",_0: "incorrect",_1: _U.eq(square.visibility,Incorrect)}
                                                       ,{ctor: "_Tuple2",_0: "covered",_1: _U.eq(square.visibility,Covered)}
                                                       ,{ctor: "_Tuple2",_0: "peek",_1: _U.eq(square.visibility,Peek)}
                                                       ,{ctor: "_Tuple2",_0: "uncovered",_1: _U.eq(square.visibility,Uncovered)}
                                                       ,{ctor: "_Tuple2",_0: "mine",_1: _U.eq(square.content,Mine)}
                                                       ,{ctor: "_Tuple2",_0: "exploded-mine",_1: _U.eq(square.content,ExplodedMine)}
                                                       ,{ctor: "_Tuple2",_0: "touching touching0",_1: _U.eq(square.content,Touching(0))}
                                                       ,{ctor: "_Tuple2",_0: "touching touching1",_1: _U.eq(square.content,Touching(1))}
                                                       ,{ctor: "_Tuple2",_0: "touching touching2",_1: _U.eq(square.content,Touching(2))}
                                                       ,{ctor: "_Tuple2",_0: "touching touching3",_1: _U.eq(square.content,Touching(3))}
                                                       ,{ctor: "_Tuple2",_0: "touching touching4",_1: _U.eq(square.content,Touching(4))}
                                                       ,{ctor: "_Tuple2",_0: "touching touching5",_1: _U.eq(square.content,Touching(5))}
                                                       ,{ctor: "_Tuple2",_0: "touching touching6",_1: _U.eq(square.content,Touching(6))}
                                                       ,{ctor: "_Tuple2",_0: "touching touching7",_1: _U.eq(square.content,Touching(7))}
                                                       ,{ctor: "_Tuple2",_0: "touching touching8",_1: _U.eq(square.content,Touching(8))}]));
      var squareAttributes = _U.eq(model.state,Started) && (_U.eq(square.content,Touching(0)) && _U.eq(square.visibility,
      Uncovered)) ? _U.list([classes]) : A2(squareNeighborFlagsEqualTouchingCount,model,square) ? _U.list([classes
                                                                                                          ,uncoverNeighborsHandler]) : A2(shouldAddPeekHandlers,
      model,
      square) ? _U.list([classes,peekDownHandler,peekUpHandler]) : _U.eq(model.state,Started) ? _U.list([classes,uncoverSquareHandler]) : _U.list([classes]);
      return A2($Html.div,squareAttributes,_U.list([$Html.text(marker(square))]));
   });
   var printRow = F3(function (address,model,squares) {
      return A2($Html.div,_U.list([$Html$Attributes.$class("row")]),A2($List.map,A2(printSquare,address,model),squares));
   });
   var boardView = F2(function (address,model) {
      var gameInfo = A2($Html.ul,
      _U.list([$Html$Attributes.$class("game-info")]),
      _U.list([A2($Html.li,_U.list([]),_U.list([$Html.text("Left click to uncover a square.")]))
              ,A2($Html.li,_U.list([]),_U.list([$Html.text("Click and hold on an uncovered square to peek at which neighbors it touches.")]))
              ,A2($Html.li,_U.list([]),_U.list([$Html.text("⌘ click to plant a flag over a suspected mine. ⌘ click again to make it a question mark.")]))
              ,A2($Html.li,_U.list([]),_U.list([$Html.text("The game is won when all squares not containing mines have been uncovered.")]))]));
      var controlRow = A2($Html.div,
      _U.list([$Html$Attributes.$class("row controls")]),
      _U.list([score(model)
              ,A2(restartButton,address,model)
              ,A2($Html.div,_U.list([$Html$Attributes.$class("clock")]),_U.list([$Html.text(padNumber(model.elapsedTime))]))]));
      var boardRows = A2($Html.div,_U.list([]),A2($List.map,A2(printRow,address,model),model.board));
      return A2($Html.div,_U.list([]),A2($List._op["::"],controlRow,A2($List._op["::"],boardRows,A2($List._op["::"],gameInfo,_U.list([])))));
   });
   var view = F2(function (address,model) {
      var boardClasses = $Html$Attributes.classList(_U.list([{ctor: "_Tuple2",_0: "board",_1: true}
                                                            ,{ctor: "_Tuple2",_0: $String.toLower($Basics.toString(model.state)),_1: true}
                                                            ,{ctor: "_Tuple2",_0: $String.toLower($Basics.toString(model.mode)),_1: true}]));
      var board = function () {
         var _p21 = model.state;
         if (_p21.ctor === "Pending") {
               return A2(pendingView,address,model);
            } else {
               return A2(boardView,address,model);
            }
      }();
      return A2($Html.div,_U.list([boardClasses]),_U.list([board]));
   });
   var linkedTouchingNeighbors = F3(function (model,pos,neighborsDict) {
      if (A2($Dict.member,pos,neighborsDict)) return neighborsDict; else {
            var squareIsTouching0 = A2($Maybe.withDefault,
            false,
            A2($Maybe.map,
            function (square) {
               return _U.eq(square.content,Touching(0));
            },
            $List.head(A2($List.filter,function (square) {    return A2(eqSquare,square.pos,pos);},$List.concat(model.board)))));
            if (squareIsTouching0) {
                  var neighbors$ = A2(F2(function (x,y) {    return A2($List._op["::"],x,y);}),pos,A2(neighbors,model.dimensions,pos));
                  var neighborIsTouching = function (square) {
                     var _p22 = square.content;
                     if (_p22.ctor === "Touching") {
                           return A2($List.member,square.pos,neighbors$);
                        } else {
                           return false;
                        }
                  };
                  var neighborPositions = A2($List.map,function (_) {    return _.pos;},A2($List.filter,neighborIsTouching,$List.concat(model.board)));
                  var currentDict = A3($Dict.insert,pos,neighborPositions,neighborsDict);
                  var neighborsDict$ = A3($List.foldl,linkedTouchingNeighbors(model),currentDict,neighbors$);
                  return neighborsDict$;
               } else return neighborsDict;
         }
   });
   var uncoverSquareAndNeighbors = F2(function (model,square) {
      var linkedNeighbors = $List.concat($Set.toList($Set.fromList($Dict.values(A3(linkedTouchingNeighbors,model,square.pos,$Dict.empty)))));
      var findNeighborSquares = function (square$) {    return A2($List.member,square$.pos,linkedNeighbors);};
      var neighborSquares = A2($List.filter,findNeighborSquares,$List.concat(model.board));
      var updateRow = function (row) {    return A2($List.map,uncoverMatchingSquares(neighborSquares),row);};
      var updatedBoard = A2($List.map,updateRow,model.board);
      return _U.update(model,{board: updatedBoard});
   });
   var updateSquareSelection = F2(function (model,square) {
      var _p23 = square.content;
      switch (_p23.ctor)
      {case "Touching": if (_p23._0 === 0) {
                 return A2(uncoverSquareAndNeighbors,model,square);
              } else {
                 var flaggedCount = A2(flaggedNeighboringMineCount,model,square);
                 return _U.eq(_p23._0,flaggedCount) ? A2(uncoverSquareAndNeighbors,model,square) : A2(uncoverSquareInBoard,model,square);
              }
         case "Mine": return A2(mineExploded,model,square);
         default: return model;}
   });
   var generateBoard = F3(function (time,_p24,mineCount) {
      var _p25 = _p24;
      var _p27 = _p25._0;
      var _p26 = _p25._1;
      var totalSquareCount = _p27 * _p26;
      var mineSquarePositions = $Set.toList(A2($Set.map,
      cellNumberToSquarePosition(_p27),
      A4(generateRandomMineCells,mineCount,totalSquareCount,$Basics.floor(time),$Set.empty)));
      var mineNeighbors = A3($List.foldl,$List.append,_U.list([]),A2($List.map,neighbors({ctor: "_Tuple2",_0: _p27,_1: _p26}),mineSquarePositions));
      var makeRow = F3(function (row,squaresPerRow,rowSquares) {
         makeRow: while (true) if (_U.eq($List.length(rowSquares),squaresPerRow)) return rowSquares; else {
               var nextIndex = $List.length(rowSquares);
               var squarePos = {ctor: "_Tuple2",_0: row,_1: nextIndex};
               var content = function () {
                  if (A2($List.member,squarePos,mineSquarePositions)) return Mine; else {
                        var touchingCount = $List.length(A2($List.filter,function (pos) {    return _U.eq(pos,squarePos);},mineNeighbors));
                        return Touching(touchingCount);
                     }
               }();
               var square = A3(Square,squarePos,content,Covered);
               var _v21 = row,_v22 = squaresPerRow,_v23 = A2($List.append,rowSquares,_U.list([square]));
               row = _v21;
               squaresPerRow = _v22;
               rowSquares = _v23;
               continue makeRow;
      ...
      var makeRows = F3(function (rowCount,squaresPerRow,rows) {
         makeRows: while (true) if (_U.eq($List.length(rows),rowCount)) return rows; else {
               var nextIndex = $List.length(rows);
               var row = A3(makeRow,nextIndex,squaresPerRow,_U.list([]));
               var _v24 = rowCount,_v25 = squaresPerRow,_v26 = A2($List.append,rows,_U.list([row]));
               rowCount = _v24;
               squaresPerRow = _v25;
               rows = _v26;
               continue makeRows;
      ...
      return A3(makeRows,_p26,_p27,_U.list([]));
   });
   var newGame = function (model) {
      return _U.update(model,{state: Started,board: A3(generateBoard,model.currentTime,model.dimensions,model.mineCount),startTime: 0,elapsedTime: 0});
   };
   var update = F2(function (action,model) {
      var _p28 = action;
      switch (_p28.ctor)
      {case "ModeSelect": var model$ = A2(selectMode,_p28._0,model);
           var board = A3(generateBoard,model.currentTime,model$.dimensions,model$.mineCount);
           return _U.update(model$,{state: Started,board: board});
         case "Restart": return newGame(model);
         case "SelectSquare": var _p29 = _p28._0;
           return model.metaKeyDown ? initializeStartTime(A2(flagSquareInBoard,model,_p29)) : initializeStartTime(promoteToWinOrLoss(A2(updateSquareSelection,
           model,
           _p29)));
         case "PeekSquare": return A2(peekSquareNeighbors,model,_p28._0);
         case "UncoverNeighborSquares": return initializeStartTime(promoteToWinOrLoss(A2(uncoverSquareNeighbors,model,_p28._0)));
         case "MetaKeyDown": return _U.update(model,{metaKeyDown: _p28._0});
         default: return updateElapsedTime(_U.update(model,{currentTime: _p28._0}));}
   });
   return _elm.Games.Minesweeper.values = {_op: _op,view: view,update: update,init: init,inputs: inputs,Model: Model};
};
Elm.Games = Elm.Games || {};
Elm.Games.TicTacToe = Elm.Games.TicTacToe || {};
Elm.Games.TicTacToe.make = function (_elm) {
   "use strict";
   _elm.Games = _elm.Games || {};
   _elm.Games.TicTacToe = _elm.Games.TicTacToe || {};
   if (_elm.Games.TicTacToe.values) return _elm.Games.TicTacToe.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $UI = Elm.UI.make(_elm);
   var _op = {};
   var playerName = function (player) {    var _p0 = player;if (_p0.ctor === "X") {    return "X";} else {    return "O";}};
   var cssClassForPlayer = function (player) {    return A2($Basics._op["++"],"player-",$String.toLower($Basics.toString(player)));};
   var gameState = function (model) {
      var coloredPlayerName = function (player) {
         return A2($Html.span,_U.list([$Html$Attributes.$class(cssClassForPlayer(player))]),_U.list([$Html.text(playerName(player))]));
      };
      var _p1 = model.state;
      switch (_p1.ctor)
      {case "Pending": return A2($Html.p,
           _U.list([]),
           _U.list([$Html.text("Waiting on player "),coloredPlayerName(model.currentPlayer),$Html.text(" to begin...")]));
         case "Started": return A2($Html.p,_U.list([]),_U.list([$Html.text("Player "),coloredPlayerName(model.currentPlayer),$Html.text(", it is your turn")]));
         case "Winner": return A2($Html.p,_U.list([]),_U.list([$Html.text("Player "),coloredPlayerName(_p1._0),$Html.text(" won!")]));
         default: return $Html.text("Aww shucks, stalemate!");}
   };
   var row = function (children) {    return A2($Html.div,_U.list([$Html$Attributes.$class("row")]),children);};
   var boardCssClass = function (state) {
      var _p2 = state;
      if (_p2.ctor === "Winner") {
            return "winner";
         } else {
            return $String.toLower($Basics.toString(state));
         }
   };
   var winningCombinations = function (p) {
      return _U.list([{ctor: "_Tuple3",_0: {ctor: "_Tuple3",_0: p,_1: 0,_2: 0},_1: {ctor: "_Tuple3",_0: p,_1: 0,_2: 1},_2: {ctor: "_Tuple3",_0: p,_1: 0,_2: 2}}
                     ,{ctor: "_Tuple3",_0: {ctor: "_Tuple3",_0: p,_1: 1,_2: 0},_1: {ctor: "_Tuple3",_0: p,_1: 1,_2: 1},_2: {ctor: "_Tuple3",_0: p,_1: 1,_2: 2}}
                     ,{ctor: "_Tuple3",_0: {ctor: "_Tuple3",_0: p,_1: 2,_2: 0},_1: {ctor: "_Tuple3",_0: p,_1: 2,_2: 1},_2: {ctor: "_Tuple3",_0: p,_1: 2,_2: 2}}
                     ,{ctor: "_Tuple3",_0: {ctor: "_Tuple3",_0: p,_1: 0,_2: 0},_1: {ctor: "_Tuple3",_0: p,_1: 1,_2: 1},_2: {ctor: "_Tuple3",_0: p,_1: 2,_2: 2}}
                     ,{ctor: "_Tuple3",_0: {ctor: "_Tuple3",_0: p,_1: 0,_2: 2},_1: {ctor: "_Tuple3",_0: p,_1: 1,_2: 1},_2: {ctor: "_Tuple3",_0: p,_1: 2,_2: 0}}
                     ,{ctor: "_Tuple3",_0: {ctor: "_Tuple3",_0: p,_1: 0,_2: 0},_1: {ctor: "_Tuple3",_0: p,_1: 1,_2: 0},_2: {ctor: "_Tuple3",_0: p,_1: 2,_2: 0}}
                     ,{ctor: "_Tuple3",_0: {ctor: "_Tuple3",_0: p,_1: 0,_2: 1},_1: {ctor: "_Tuple3",_0: p,_1: 1,_2: 1},_2: {ctor: "_Tuple3",_0: p,_1: 2,_2: 1}}
                     ,{ctor: "_Tuple3"
                      ,_0: {ctor: "_Tuple3",_0: p,_1: 0,_2: 2}
                      ,_1: {ctor: "_Tuple3",_0: p,_1: 1,_2: 2}
                      ,_2: {ctor: "_Tuple3",_0: p,_1: 2,_2: 2}}]);
   };
   var didPlayerWin = F2(function (player,selections) {
      var combinations = winningCombinations(player);
      var didPlayerWinCombination = function (combination) {
         var _p3 = combination;
         var nodes = {ctor: "_Tuple3",_0: A2($List.member,_p3._0,selections),_1: A2($List.member,_p3._1,selections),_2: A2($List.member,_p3._2,selections)};
         var _p4 = nodes;
         if (_p4.ctor === "_Tuple3" && _p4._0 === true && _p4._1 === true && _p4._2 === true) {
               return true;
            } else {
               return false;
            }
      };
      return A2($List.any,didPlayerWinCombination,combinations);
   });
   var isStalemate = function (selections) {    return _U.eq($List.length(selections),9);};
   var Model = F3(function (a,b,c) {    return {selections: a,state: b,currentPlayer: c};});
   var Stalemate = {ctor: "Stalemate"};
   var Winner = function (a) {    return {ctor: "Winner",_0: a};};
   var newStateFromUpdatedSelections = F3(function (selections,state,player) {
      return A2(didPlayerWin,player,selections) ? Winner(player) : isStalemate(selections) ? Stalemate : state;
   });
   var Started = {ctor: "Started"};
   var Pending = {ctor: "Pending"};
   var Restart = {ctor: "Restart"};
   ...
   var O = {ctor: "O"};
   var X = {ctor: "X"};
   var init = A3(Model,_U.list([]),Pending,X);
   var squareBuilder = F4(function (address,model,row,col) {
      var idAttr = $Html$Attributes.id(A2($Basics._op["++"],
      "square-",
      A2($Basics._op["++"],$Basics.toString(row),A2($Basics._op["++"],"-",$Basics.toString(col)))));
      var oIsSelected = A2($List.member,{ctor: "_Tuple3",_0: O,_1: row,_2: col},model.selections);
      var xIsSelected = A2($List.member,{ctor: "_Tuple3",_0: X,_1: row,_2: col},model.selections);
      var isSelectable = (_U.eq(model.state,Pending) || _U.eq(model.state,Started)) && ($Basics.not(xIsSelected) || $Basics.not(oIsSelected));
      var classes = $Html$Attributes.classList(_U.list([{ctor: "_Tuple2",_0: "square",_1: true}
                                                       ,{ctor: "_Tuple2",_0: "row-item",_1: true}
                                                       ,{ctor: "_Tuple2",_0: "selectable",_1: isSelectable}
                                                       ,{ctor: "_Tuple2",_0: "selected",_1: xIsSelected || oIsSelected}
                                                       ,{ctor: "_Tuple2",_0: "player-x",_1: xIsSelected}
                                                       ,{ctor: "_Tuple2",_0: "player-o",_1: oIsSelected}]));
      var disallowOnClick = $Basics.not(_U.eq(model.state,Pending)) && $Basics.not(_U.eq(model.state,Started)) || (xIsSelected || oIsSelected);
      var onClickAttribute = $Basics.not(disallowOnClick) ? _U.list([A2($Html$Events.onClick,
      address,
      SelectSquare({ctor: "_Tuple3",_0: model.currentPlayer,_1: row,_2: col}))]) : _U.list([]);
      var attributes = A2($List._op["::"],idAttr,A2($List._op["::"],classes,onClickAttribute));
      return A2($Html.div,attributes,_U.list([$Html.text("")]));
   });
   var view = F2(function (address,model) {
      var boardClasses = $Html$Attributes.classList(_U.list([{ctor: "_Tuple2",_0: "board",_1: true}
                                                            ,{ctor: "_Tuple2",_0: boardCssClass(model.state),_1: true}]));
      var restartButton = A2($Html.div,
      _U.list([$Html$Attributes.$class("restart-button row-item"),$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "text-align",_1: "center"}]))]),
      _U.list([A2($Html.p,_U.list([]),_U.list([A2($UI.pureButton,A2($Html$Events.onClick,address,Restart),"Restart Game")]))]));
      var currentGameState = A2($Html.div,_U.list([$Html$Attributes.$class("game-state row-item")]),_U.list([gameState(model)]));
      var lastRow = A2($Html.div,_U.list([$Html$Attributes.$class("game-controls row")]),_U.list([currentGameState,restartButton]));
      var squaresRow3 = row(_U.list([A4(squareBuilder,address,model,2,0),A4(squareBuilder,address,model,2,1),A4(squareBuilder,address,model,2,2)]));
      var squaresRow2 = row(_U.list([A4(squareBuilder,address,model,1,0),A4(squareBuilder,address,model,1,1),A4(squareBuilder,address,model,1,2)]));
      var squaresRow1 = row(_U.list([A4(squareBuilder,address,model,0,0),A4(squareBuilder,address,model,0,1),A4(squareBuilder,address,model,0,2)]));
      var title = A2($Html.h1,_U.list([$Html$Attributes.$class("title")]),_U.list([$Html.text("Tic Tac Toe")]));
      var boardRows = A2($List._op["::"],title,A2($Basics._op["++"],_U.list([squaresRow1,squaresRow2,squaresRow3]),_U.list([lastRow])));
      return A2($Html.div,_U.list([boardClasses]),boardRows);
   });
   var nextPlayer = function (player) {    var _p5 = player;if (_p5.ctor === "X") {    return O;} else {    return X;}};
   var update = F2(function (action,model) {
      var _p6 = action;
      if (_p6.ctor === "SelectSquare") {
            var nextSelections = A2($List._op["::"],_p6._0,model.selections);
            var nextState = function () {
               var _p7 = model.state;
               switch (_p7.ctor)
               {case "Pending": return Started;
                  case "Started": return A3(newStateFromUpdatedSelections,nextSelections,model.state,model.currentPlayer);
                  default: return model.state;}
            }();
            return _U.update(model,{currentPlayer: nextPlayer(model.currentPlayer),state: nextState,selections: nextSelections});
         } else {
            return init;
         }
   });
   return _elm.Games.TicTacToe.values = {_op: _op,view: view,update: update,init: init,Model: Model};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   $Games$Minesweeper = Elm.Games.Minesweeper.make(_elm),
   $Games$TicTacToe = Elm.Games.TicTacToe.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   $Html$Events = Elm.Html.Events.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $UI = Elm.UI.make(_elm);
   var _op = {};
   var noGameView = A2($Html.div,_U.list([$Html$Attributes.$class("no-game")]),_U.list([$Html.text("Select your game above to get started!")]));
   var update = F2(function (action,model) {
      var model$ = function () {
         var _p0 = action;
         switch (_p0.ctor)
         {case "ChangeGame": return _U.update(model,{selectedGame: _p0._0});
            case "MinesweeperEvent": return _U.update(model,{minesweeperModel: A2($Games$Minesweeper.update,_p0._0,model.minesweeperModel)});
            default: return _U.update(model,{ticTacToeModel: A2($Games$TicTacToe.update,_p0._0,model.ticTacToeModel)});}
      }();
      return A2(F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),model$,$Effects.none);
   });
   var Model = F3(function (a,b,c) {    return {selectedGame: a,minesweeperModel: b,ticTacToeModel: c};});
   var TicTacToeEvent = function (a) {    return {ctor: "TicTacToeEvent",_0: a};};
   var MinesweeperEvent = function (a) {    return {ctor: "MinesweeperEvent",_0: a};};
   var inputs = _U.list([A2($Signal.map,MinesweeperEvent,$Games$Minesweeper.inputs)]);
   var ChangeGame = function (a) {    return {ctor: "ChangeGame",_0: a};};
   var TicTacToe = {ctor: "TicTacToe"};
   var Minesweeper = {ctor: "Minesweeper"};
   var gameSelectorMenu = function (address) {
      var ticTacToeButton = A2($UI.pureButton,A2($Html$Events.onClick,address,ChangeGame(TicTacToe)),"Tic-Tac-Toe");
      var minesweeperButton = A2($UI.pureButton,A2($Html$Events.onClick,address,ChangeGame(Minesweeper)),"Minesweeper");
      return A2($Html.nav,_U.list([$Html$Attributes.$class("game-selector-menu")]),_U.list([minesweeperButton,ticTacToeButton]));
   };
   var NoGame = {ctor: "NoGame"};
   var init = function () {
      var model = A3(Model,NoGame,$Games$Minesweeper.init,$Games$TicTacToe.init);
      return A2(F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),model,$Effects.none);
   }();
   var view = F2(function (address,model) {
      var gameView = function () {
         var _p1 = model.selectedGame;
         switch (_p1.ctor)
         {case "NoGame": return noGameView;
            case "Minesweeper": return A2($Games$Minesweeper.view,A2($Signal.forwardTo,address,MinesweeperEvent),model.minesweeperModel);
            default: return A2($Games$TicTacToe.view,A2($Signal.forwardTo,address,TicTacToeEvent),model.ticTacToeModel);}
      }();
      var gameWrapperClasses = $Html$Attributes.classList(_U.list([{ctor: "_Tuple2",_0: "game",_1: true}
                                                                  ,{ctor: "_Tuple2",_0: "game-selector",_1: _U.eq(model.selectedGame,NoGame)}
                                                                  ,{ctor: "_Tuple2",_0: "minesweeper",_1: _U.eq(model.selectedGame,Minesweeper)}
                                                                  ,{ctor: "_Tuple2",_0: "tic-tac-toe",_1: _U.eq(model.selectedGame,TicTacToe)}]));
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.header,
              _U.list([]),
              _U.list([A2($Html.h1,_U.list([$Html$Attributes.$class("title")]),_U.list([$Html.text("@localshred Gaming, Inc.")])),gameSelectorMenu(address)]))
              ,A2($Html.div,_U.list([gameWrapperClasses]),_U.list([gameView]))
              ,A2($Html.footer,
              _U.list([]),
              _U.list([A2($Html.p,
              _U.list([]),
              _U.list([$Html.text("Cobbled together by ")
                      ,A2($Html.a,_U.list([$Html$Attributes.href("https://twitter.com/localshred")]),_U.list([$Html.text("@localshred")]))
                      ,$Html.text(" (")
                      ,A2($Html.a,_U.list([$Html$Attributes.href("https://github.com/localshred/tic-tac-toe/")]),_U.list([$Html.text("View Source")]))
                      ,$Html.text(")")]))]))]));
   });
   ...
   return _elm.Main.values = {_op: _op
                             ...
                             ,MinesweeperEvent: MinesweeperEvent
                             ,TicTacToeEvent: TicTacToeEvent
                             ,Model: Model
                             ,main: main
                             ,app: app
                             ,inputs: inputs
                             ...
                             ,update: update
                             ,view: view
                             ,gameSelectorMenu: gameSelectorMenu
                             ,noGameView: noGameView};
};
